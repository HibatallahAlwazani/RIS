...
Ntab=[10:10:100]; % consider ts tab power tab, l tab, snr 
for n=1:length(Ntab)
     N=Ntab(n)
   ...
   %  lambda/4
    ...
    [p0ts p1ts2]=Match_probability(Q,sigma,P,channelgain,Ts2); %approximate prob, true probability
  ...
  if p0>=0.5
   ...
   Rk=(1-Hb)*log2(Q)/(Ts/2);   
    ...
    Hbts=-p1ts*log2(p1ts)-(1-p1ts)*log2(1-p1ts);
    Rk_ts=(1-Hbts)*log2(Q)/(Ts2/2);
...
Qstar=Qtab(I);
   ...
   % Qstar2=optimQ2(sigmasq);
  %[p0q2 p1q2]=Match_probability(Qstar2,sigma,P,channelgain,Ts); %approximate prob, true probability
  %  pq2=p1q;
   %  Hbq2=-pq2*log2(pq2)-(1-pq2)*log2(1-pq2);
   % Rk_Qoptim2=(1-Hbq)*log2(Qstar2)/(Ts/2);
 ...
 %% Information Rate
    ...
    RIts=log2((beta_ab+beta_ar*beta_rb*trace(R*R'))*log(L2)*P/sigma);% Ts=random, tk optimal , q not 
  ...
  RInr=log2(1+(beta_ab*P)/sigma); % no RIS
    ...
    Pe=0
    %% OTP
% abs(hab+hrb*Theta_star*har)^2=(abs(hab)+abs(hrb)*abs(har))^2
%Theta_star=diag(exp(1i*(-angle(har)+angle(hrb)+angle(hab)))); %% correct expression
%v=angle(exp(1i*(-angle(har)+angle(hrb)+angle(hab))));
RIotp=log2(((abs(hab)+abs(hrb)'*abs(har))^2*P)/sigma); % their paper %*log( Lotp)
    ...
    RsTH(n)=(1-Pe)*min(Tk_otp/(T)*Rk_lb,(T-Tk_otp)/T*RIth); %Rith tkith
    Rsts(n)=(1-Pe)*min(Tk/(T)*Rk_ts,(T-Tk)/T*RIts);
       Rs_optimm(n)=(1-Pe)*min(Tk_trial/(T)*Rk,(T-Tk_trial)/T*RI); 
     %    Rs_optimmQ1(n)=(1-Pe)*min(Tk_trialq2/(T)*Rk_Qoptim2,(T-Tk_trialq2)/T*RIq2); 
...
plot(Ntab,RsTH2,'k-o','LineWidth',2)
plot(Ntab,RsTH,'k--','LineWidth',2)
plot(Ntab,Rs_optimm2Q,'b-o','LineWidth',2)
%plot(Ntab,Rs_optimmQ1,'b-o','LineWidth',2)
plot(Ntab,Rs_optimm,'b--','LineWidth',2)
%plot(Ntab,Rs,'b-.','LineWidth',2)
plot(Ntab,Rsts,'b-','LineWidth',2)
plot(Ntab,RsNR,'r-','LineWidth',2)
%all tk optimal
%Q optimal'
legend('SKR Th.,  optimal T_k','SKR Th., Random T_k','Practical SKR, Optimized','Practical SKR, T_k^*,  T_s=2, Q=32','Random Tk, Q=32, Ts=8','No RIS' )
